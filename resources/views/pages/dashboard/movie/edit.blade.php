<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Movie &raquo; {{ $movie->title }} &raquo; Edit
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div>
                ...
                @endif
                <form class="w-full" action="{{ route('dashboard.movie.update', $movie->id) }}" method="post"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input value="{{ old('title') ?? $movie->title }}" name="title"
                            ...
                            <select name="genres_id"
                                ...
                                @foreach ($genres as $genre)
                                    <option value="{{ $genre->id }}"
                                        {{ $movie->genres_id == $genre->id ? 'selected' : '' }}>
                                        {{ $genre->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input value="{{ old('release_year') ?? $movie->release_year }}" name="release_year"
                                class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                        ...
                        <div class="w-full px-3">
                            ...
                            <div class="w-full mb-4">
                                ...
                                <textarea id="description" name="description" rows="8"
                                    class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                    placeholder="Write an description...">{{ old('description') ?? $movie->description }}</textarea>
